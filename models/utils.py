...
def get_device(x: Union[torch.Tensor, Sequence[torch.Tensor]]) -> int:
    if isinstance(x, torch.Tensor):
        b_s = x.device
    else:
        b_s = x[0].device
...
def generate_padding_mask(sequences: Union[torch.Tensor, None], padding_idx: int) -> torch.BoolTensor:
    '''
